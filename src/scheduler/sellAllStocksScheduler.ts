...
const sellAllStocksScheduler = async () => {
  schedule.scheduleJob("01 9,15 * * 1-5", async function () {
    ...
    try {
      ...
      await retryOperation(
        async () => {
          const object = new TradingSettlementDetail.Builder(
            getKoreaDate(),
            accessToken,
            process.env.SYMBOL_INVERSE as string,
            "00",
            "02"
          ).build();
          ...
          if (result.output[0]) {
            ...
            for (const element of result.output) {
              const object = new RetradingOrder.Builder(
                accessToken,
                String(element.odno),
                "01",
                "02",
                String(element.rmn_qty),
                "0"
              ).build();
              await object.handle();
              await sleep(1000);
  ...
  console.log(
    "전량 매도 스케줄러가 설정되었습니다. 월-금 9:01과 15:01에 실행됩니다."
  );
};
